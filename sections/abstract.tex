...
Symbolic planners provide systematic search and formal guarantees, but their applicability depends on the environment being expressed in the planner's predefined vocabulary. In open-world settings, a robot may perceive an object that can fulfill a required functional role without that object being assigned the corresponding symbolic type. The resulting \emph{typing gap} can make a task appear unsolvable even when a suitable physical substitute is available. We introduce CAST (Constrained Assignment of Symbolic Types), a neuro-symbolic framework that uses a large language model (LLM) to assign available objects to existing symbolic types at runtime. CAST restricts the LLM to modifying the object-to-type assignment while leaving the rest of the planning instance unchanged. We evaluate CAST using three LLM-querying strategies with open-weight models on problems requiring single and simultaneous substitutions, robustness to distractors and semantic variations, and refusal when no valid substitute exists. The typing-based methods substantially outperform an end-to-end LLM planning baseline on solvable substitution problems. Our results also expose a recovery--refusal trade-off: iterative refinement provides strong plan recovery at low token cost, whereas consensus and judge-based strategies more reliably reject semantically invalid substitutions. Finally, a physical-robot deployment demonstrates that CAST composes with an open-vocabulary perception-to-planning pipeline.
