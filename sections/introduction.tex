...
% Tokens
% $VMMgg9LKFaHX7GZjt9q-IBZ9lOPGuPXaUYMj1FtAeMY$
% $TsfrHQQAXkmDvnixJk1y1_gx5h25PmSJXdAtXQJkEF0$
Robots operating in real-world deployments must operate under conditions that cannot be fully anticipated at design time~\cite{wray2025requirements}. Expected tools may be missing, unfamiliar objects may be present, and the available resources may support ways
...
domain's predefined vocabulary. A runtime perception system may recognize
...
unconstrained baseline inspired by \citet{gragera2023exploring} that lets the LLM rewrite the PDDL directly, on tasks
